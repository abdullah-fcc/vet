// ============================================
// VetAssist AI — Express App (shared between local & Vercel)
// ============================================
...
// Bypass TLS cert validation in development
...
// Creates a Retell web call and returns access token
// ============================================
  ...
  try {
    ...
    if (!effectiveAgentId) {
      return res.status(400).json({
        error: 'No agent ID provided. Pass agentId in the request body or set RETELL_AGENT_ID in .env.',
      });
    }
...
app.post('/api/retell/process-call', async (req, res) => {
  try {
    ...
    console.log('[Call Processed]', callId, callData.call_status);
